...
function print_verbose_param_output(data; model_name::String="")
    ...
    return pretty_table(
        ...
        style=TextTableStyle(
            first_line_column_label=crayon"white bold",
            title=crayon"magenta bold"
        ),
        table_format=TextTableFormat(borders=text_table_borders__unicode_rounded),
        title="[$model_name model]",
        subtitle="validity of equilibrium parameters",
        title_alignment=:c,
...
function _print_results_to_stdout(MLPs::Vector{MLP_Model}; kwargs...)
    ...
    if verbose && eq_betaN > 0
        ...
        pretty_table(
            ...
            table_format=TextTableFormat(borders=text_table_borders__unicode_rounded),
            title="[MLP model]",
            subtitle=@sprintf("(Equilibrium βₙ=%.2f)", eq_betaN),
            title_alignment=:c,
            subtitle_alignment=:c
        )
    else
